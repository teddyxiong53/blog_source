...
`AddMatch` 是 D-Bus 中用于添加匹配规则的一种方法。

这些规则定义了哪些消息应该被接收，这对于**过滤特定的 D-Bus 消息**非常有用。



通过调用 `AddMatch` 方法，您可以指定一个匹配字符串，该字符串用于过滤来自 D-Bus 总线的消息。

匹配字符串遵循特定的语法，允许您根据接口、信号、名称和其他属性来筛选消息。

# dbus_threads_init_default

在使用 D-Bus 的多线程应用程序中，通常需要在任何 D-Bus 函数调用之前调用 `dbus_threads_init_default`。这确保了 D-Bus 正确设置了其内部锁机制，使得在不同线程中安全地调用 D-Bus API 成为可能。

`dbus_threads_init_default` 不是在所有情况下都必须调用，但在多线程环境中使用 D-Bus 时，建议调用它以确保线程安全。以下是一些关键点，帮助你理解何时需要调用这个函数：

### 何时需要调用

1. **多线程环境**：如果你的应用程序使用多个线程并且你在这些线程中调用 D-Bus API，那么应该调用 `dbus_threads_init_default`。这样可以确保 D-Bus 的内部状态正确设置，以支持多线程。
