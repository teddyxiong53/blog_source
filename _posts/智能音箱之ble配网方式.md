...
配网发送广播
这个是设置随机地址：
hcitool -i hci0 cmd 0x08 0x0005 xx xx xx xx xx xx 
然后是发送广播数据。
hcitool -i hci0 cmd 0x08 0x0008
广播数据是21字节。
内容：
0x15 长度
02 01 06 11 07 这个是flag？代表了什么？
这里可以分为2个部分：02 01 06 里，02表示长度，后面是01 06，01 06是什么意思？
表示ble发现使能。
11表示17个字节，第一个字节的07，然后是16个字节的service uuid。
