...
1. 真无线耳机
   ...
   - 典型场景是 1 个 CIG 包含 2 个 CIS（左耳和右耳），远低于 31 个上限。
   - 如果耳机支持额外功能（如麦克风回传语音），可能增加到 3 个 CIS。

2. 多用户音频共享

   ：

   - 一个中心设备（如智能手机）可以与多个耳机建立 CIS（如 5 个用户共享音乐），仍远未达到 31 个上限。

3. 专业音频设备

   ：

   - 在录音棚或现场演出中，一个 CIG 可以管理多个麦克风、扬声器或监听设备的音频流，接近或达到 31 个 CIS。

4. 未来扩展

   ：

   - 31 个 CIS 为未来的复杂音频应用预留了空间，如虚拟现实（VR）或增强现实（AR）中多声道沉浸式音频。
...
**答案：可以，但取决于具体实现方式和应用需求。**

在蓝牙 LE Audio 中，**CIS（Connected Isochronous Stream）** 是中心设备（Central）和外围设备（Peripheral）之间的一个逻辑传输通道，用于承载等时数据（如音频）。

蓝牙核心规范并未强制规定一个 CIS 只能传输单声道数据，

而是允许其承载多个声道的数据，

前提是数据包的带宽和配置能够支持。

**CIS 的定义**：CIS 是一个双向的等时数据流，传输的数据内容由上层协议（如 LC3 编解码器）和应用层决定。规范中没有限制一个 CIS 只能传输一个声道。

**SDU（Service Data Unit）**：CIS 传输的数据以 SDU 为单位，每个 SDU 可以包含任意格式的数据（由应用程序定义），只要不超过最大 SDU 大小（Max_SDU，例如 240 字节）。

**带宽分配**：一个 CIS 的带宽由其 QoS 参数（如 SDU Interval、Max SDU Size）决定。如果带宽足够，一个 SDU 可以打包多个声道的数据。
...
- **简化同步**：多个声道打包在一个 CIS 中，天然保证了声道间的同步，无需额外的 CIG 管理。
- **减少资源占用**：相比为每个声道分配一个 CIS，使用一个 CIS 传输多声道数据减少了链路层的事件（Events）和子事件（Subevents），降低了中心设备的调度负担。
- **带宽效率**：通过高效编码（如 LC3 的联合立体声编码），一个 CIS 可以充分利用可用带宽。

#### 缺点

- **带宽限制**：一个 CIS 的最大带宽受限于其 QoS 配置。如果声道数过多（如 5.1 或 7.1 系统），可能超出 SDU 大小或间隔的承载能力。
- **灵活性降低**：如果多个声道绑定在一个 CIS 中，难以独立调整某个声道的传输参数（如延迟或重传次数）。
...
# LC3 10ms和7.5ms帧长度和iso interval关系

对于 48kHz 采样率：

- 7.5ms 帧 = 48,000 * 0.0075 = 360 样本。
- 10ms 帧 = 48,000 * 0.01 = 480 样本。

ISO interval 是蓝牙 LE 等时通道（Isochronous Channels，包括 CIS 和 BIS）的传输参数，定义了数据包之间的时间间隔。它由蓝牙控制器的调度决定，通常在连接建立或广播配置时协商。

- **单位**: 时间槽（time slots），1 个时间槽 = 1.25ms。
...
- **来源**: 在 BlueZ 中，ISO interval 通过 HCI（Host Controller Interface）命令配置，或在 D-Bus 接口中设置。

**设计目标**: LC3 的帧持续时间通常与 ISO interval 对齐，以确保音频数据的编码、传输和解码保持同步。

**理想情况**:

- 如果 LC3_CONFIG_DURATION_7_5（7.5ms）被选中，则 ISO interval 通常配置为 7.5ms。
- 如果 LC3_CONFIG_DURATION_10（10ms）被选中，则 ISO interval 通常配置为 10ms。
...
- **含义**: 同步因子，用于计算广播同步的时间间隔。
- **值**: 2，表示同步间隔是基本间隔（通常是 ISO interval）的 2 倍。
- **上下文**: 在 BIS 中，接收端（sink）根据 SyncFactor 确定同步点的频率。例如，若 ISO interval = 10ms，则同步间隔 = 20ms。

#### 4. **Packing=0**

- **含义**: 数据包的打包方式。

- **值**: 0，通常表示“顺序打包”（Sequential Packing）。

- 上下文

  :

  - 0: Sequential（顺序），数据包按顺序排列。
  - 1: Interleaved（交错），多个流的数据包交错发送。
  - 此处为顺序打包，适合单个 BIS。
...
- 上下文
  ...
  - 0: Unframed，每个 ISO 数据包是一个独立单元。
  - 1: Framed，数据包可能包含多个子帧。
  - 未分帧适合简单的音频流传输。
...
- **含义**: 同步超时时间（单位：毫秒）。
- **值**: 2000，即 2 秒。
- **上下文**: 如果接收端（BIS sink）在 2 秒内未收到同步数据，则认为同步失败，可能断开或重试。
...
- **含义**: ISO interval（等时间隔，单位：微秒）。
- **值**: 10000，即 10,000 微秒 = 10 毫秒。
- **上下文**: 定义数据包之间的传输间隔，10ms 是 LE Audio 的常见值，与 LC3 的 10ms 帧持续时间对齐。

#### 14. **Latency=10**

- **含义**: 传输延迟（单位：毫秒）。
- **值**: 10，表示端到端延迟为 10ms。
- **上下文**: 这是预期延迟，可能包括编码、传输和解码时间，与 ISO interval 一致。



#### **PHY=2**

- **含义**: 物理层（Physical Layer）类型。

- **值**: 2，表示使用“2M PHY”（2 Mbps）。
...
- **含义**: 重传次数。
- **值**: 2，表示每个数据包最多重传 2 次。
- **上下文**: 在 CIS 中，重传提高可靠性；在 BIS 中，通常无重传，此处可能是 CIS 配置的遗留值，或表示广播中的重试机制。



# **MTU 0:80**

- **含义**: Maximum Transmission Unit（最大传输单元），表示数据包的大小限制。

- 值分解

  :

  - **0**: 输入方向的 MTU（可能是未定义或未使用）。
  - **80**: 输出方向的 MTU，表示每个数据包的最大有效载荷为 80 字节。

- **上下文**: 在蓝牙 LE Audio 中，MTU 定义了 ISO 数据包的 SDU（Service Data Unit，服务数据单元）大小。80 与之前的 QoS 参数 SDU=80 一致，表示每 10ms 传输 80 字节。
...
- **SDU**: 在之前的 QoS 配置中，SDU=80（每个 ISO 数据包 80 字节）。
...
- 分析
  ...
  - 一个 SDU（80 字节）可能包含两个 LC3 帧（40 字节 * 2 = 80 字节）。
  - 或者 SDU 配置与此日志不完全对应（例如 QoS 为 BIS 配置，而此日志为 CIS 配置）。

- **假设**: 如果 CIS sink 使用此配置，实际 SDU 可能为 40 字节，与 framelen 匹配。

#### 4. **nblks:1 的意义**

- 表示每个 ISO 数据包携带一个 LC3 帧。如果 SDU=40 字节，则与 framelen=40 一致；若 SDU=80，则可能传输两个帧（需检查具体实现）。
...
5.33ms 间隔可能是内部处理周期，10ms ISO interval 的一半，可能用于速率控制或双缓冲。
...
例如bis source。cis source。

**功能**:

- 配置音频流的 QoS（Quality of Service，服务质量）参数（如间隔、延迟、SDU 大小）。
- 启动传输通道（例如通过 BlueZ 的 D-Bus 接口或 HCI 命令）。
- 管理音频数据的编码和发送。



set_latency: BAP presentation delay 20000 us, node latency 256/48000

调整节点延迟，使其略小于呈现延迟（presentation delay）的一半，留出空间用于缓冲速率控制。
