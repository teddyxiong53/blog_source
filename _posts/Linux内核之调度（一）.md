...
CFS也是对于普通进程而言的，不包括RT进程。

当RT进程都睡眠了（或者RT进程已经跑了超过了sched_rt_runtime_us时间值），那么就该普通进程被调度了。
...
必须给普通进程留出运行时间。linux系统里存在一个RT门限。sched_rt_runtime_us	就是这个限制时间。
...
当进程数很大的时候，遍历所有进程花的时间就很长了。调度的时间复杂度是O(n)。
